...
const inputSt = { width: 80, background: 'var(--mv-surface)', border: '1px solid var(--mv-hairline)', borderRadius: 7, padding: '6px 9px', fontSize: 13, outline: 'none' };
...
export default function Switchboard() {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto' }}>
      ...
      <h2 style={{ fontSize: 19, fontWeight: 800, color: 'var(--mv-ink)', margin: '0 0 4px' }}>⏱️ SLA &amp; Autopilot Switchboard</h2>
      <p style={{ fontSize: 12.5, color: 'var(--mv-ink-52)', marginTop: 0, marginBottom: 22, maxWidth: 640, lineHeight: 1.5 }}>
        Set response-time thresholds, and promote calibrated workflows from Probation to Full Autopilot.
      </p>

      {loading && <div style={{ padding: 28, textAlign: 'center', color: 'var(--mv-ink-45)', fontSize: 13 }}>Loading…</div>}

      {!loading && (
        <>
          {/* SLA thresholds */}
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--mv-ink-45)', marginBottom: 10 }}>SLA Response Thresholds</div>
          <div className="moov-card" style={{ overflow: 'hidden', marginBottom: 36 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ textAlign: 'left', color: 'var(--mv-ink-52)', fontSize: 11 }}>
              ...
              <tbody>
                {sla.map(s => (
                  <tr key={s.workflow_group} style={{ borderTop: '1px solid var(--mv-hairline)' }}>
                    <td style={{ padding: '8px 14px', fontWeight: 600, color: 'var(--mv-ink)', textTransform: 'capitalize' }}>{s.workflow_group.replace(/_/g, ' ')}</td>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--mv-ink-45)', marginBottom: 10 }}>🤖 Autopilot Workflows (Probation → Autopilot)</div>
          {trust.length === 0 && (
            <div className="moov-card" style={{ padding: 24, textAlign: 'center', color: 'var(--mv-ink-45)', fontSize: 13 }}>
              ...
              <div key={`${w.courier_code}-${w.intent}`} className="moov-card" style={{ padding: '14px 18px', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--mv-ink)', textTransform: 'capitalize' }}>
                    ...
                    {/* progress bar */}
                    <div style={{ marginTop: 8, height: 6, width: 240, background: 'var(--mv-bg)', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: w.ready ? 'var(--mv-green)' : 'var(--mv-purple)' }} />
                    </div>
                  </div>
                  <button
                    ...
                    style={{
                      ...
                      border: w.autopilot_enabled ? '1px solid var(--mv-magenta-200)' : undefined,
                      background: w.autopilot_enabled ? 'var(--mv-magenta-100)' : undefined,
                      color: w.autopilot_enabled ? 'var(--mv-magenta-deep)' : undefined,
