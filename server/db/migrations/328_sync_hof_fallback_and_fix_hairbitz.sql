-- Migration 328: Sync account numbers and DC customer IDs including HOF-only fallbacks (e.g. Hairbitz -> HOF-0029)
-- Strictly updates existing records only. Zero inserts.
...
BEGIN
  ...
  IF v_cust_id IS NOT NULL THEN
    -- Prevent unique constraint collisions
    UPDATE customers 
  ...
  -- ── Phils Plumbing (HOF-0004) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Phils Plumbing'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Phils Plumbing'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Phils Plumbing')) || '%'
       OR LOWER('Phils Plumbing') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0004'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0004'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0004',
        dc_customer_id = 'HOF-0004',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Chipex (HOF-0015) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Chipex'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Chipex'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Chipex')) || '%'
       OR LOWER('Chipex') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0015'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0015'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0015',
        dc_customer_id = 'HOF-0015',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Clearance Car Parts Ltd (HOF-0016) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Clearance Car Parts Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Clearance Car Parts Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Clearance Car Parts Ltd')) || '%'
       OR LOWER('Clearance Car Parts Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0016'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0016'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0016',
        dc_customer_id = 'HOF-0016',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Autotag Trading Ltd (HOF-0045) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Autotag Trading Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Autotag Trading Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Autotag Trading Ltd')) || '%'
       OR LOWER('Autotag Trading Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0045'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0045'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0045',
        dc_customer_id = 'HOF-0045',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Pics On Canvas (HOF-0020) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Pics On Canvas'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Pics On Canvas'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Pics On Canvas')) || '%'
       OR LOWER('Pics On Canvas') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0020'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0020'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0020',
        dc_customer_id = 'HOF-0020',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── STAR FASHION GLOBAL LTD (HOF-0021) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('STAR FASHION GLOBAL LTD'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('STAR FASHION GLOBAL LTD'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('STAR FASHION GLOBAL LTD')) || '%'
       OR LOWER('STAR FASHION GLOBAL LTD') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0021'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0021'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0021',
        dc_customer_id = 'HOF-0021',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Bargain Zone 4u (HOF-0022) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Bargain Zone 4u'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Bargain Zone 4u'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Bargain Zone 4u')) || '%'
       OR LOWER('Bargain Zone 4u') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0022'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0022'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0022',
        dc_customer_id = 'HOF-0022',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Sinatec UK (HOF-0023) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Sinatec UK'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Sinatec UK'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Sinatec UK')) || '%'
       OR LOWER('Sinatec UK') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0023'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0023'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0023',
        dc_customer_id = 'HOF-0023',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Shiptheory (HOF-0024) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Shiptheory'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Shiptheory'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Shiptheory')) || '%'
       OR LOWER('Shiptheory') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0024'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0024'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0024',
        dc_customer_id = 'HOF-0024',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Vehicle Safety and Warning Distribution Ltd (HOF-0025) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Vehicle Safety and Warning Distribution Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Vehicle Safety and Warning Distribution Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Vehicle Safety and Warning Distribution Ltd')) || '%'
       OR LOWER('Vehicle Safety and Warning Distribution Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0025'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0025'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0025',
        dc_customer_id = 'HOF-0025',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Hairbitz Ltd (HOF-0029) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Hairbitz Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Hairbitz Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Hairbitz Ltd')) || '%'
       OR LOWER('Hairbitz Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0029'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0029'
      AND id != v_cust_id;
  ...
  -- ── Marks Tey Discount Pet Foods (HOF-0030) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Marks Tey Discount Pet Foods'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Marks Tey Discount Pet Foods'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Marks Tey Discount Pet Foods')) || '%'
       OR LOWER('Marks Tey Discount Pet Foods') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0030'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0030'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0030',
        dc_customer_id = 'HOF-0030',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── The Vapour Hut (HOF-0034) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('The Vapour Hut'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('The Vapour Hut'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('The Vapour Hut')) || '%'
       OR LOWER('The Vapour Hut') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0034'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0034'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0034',
        dc_customer_id = 'HOF-0034',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Grupoerik (HOF-0035) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Grupoerik'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Grupoerik'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Grupoerik')) || '%'
       OR LOWER('Grupoerik') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0035'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0035'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0035',
        dc_customer_id = 'HOF-0035',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── EDMUNDSON ELECTRICAL LEEDS (HOF-0039) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('EDMUNDSON ELECTRICAL LEEDS'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('EDMUNDSON ELECTRICAL LEEDS'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('EDMUNDSON ELECTRICAL LEEDS')) || '%'
       OR LOWER('EDMUNDSON ELECTRICAL LEEDS') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0039'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0039'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0039',
        dc_customer_id = 'HOF-0039',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── 3Devices ltd (HOF-0042) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('3Devices ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('3Devices ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('3Devices ltd')) || '%'
       OR LOWER('3Devices ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0042'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0042'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0042',
        dc_customer_id = 'HOF-0042',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── RICHARD WHEATLEY LTD (HOF-0018) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('RICHARD WHEATLEY LTD'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('RICHARD WHEATLEY LTD'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('RICHARD WHEATLEY LTD')) || '%'
       OR LOWER('RICHARD WHEATLEY LTD') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0018'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0018'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0018',
        dc_customer_id = 'HOF-0018',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Adrenalin 4x4 Ltd (HOF-0047) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Adrenalin 4x4 Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Adrenalin 4x4 Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Adrenalin 4x4 Ltd')) || '%'
       OR LOWER('Adrenalin 4x4 Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0047'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0047'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0047',
        dc_customer_id = 'HOF-0047',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Candy Drops (HOF-0048) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Candy Drops'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Candy Drops'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Candy Drops')) || '%'
       OR LOWER('Candy Drops') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0048'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0048'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0048',
        dc_customer_id = 'HOF-0048',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── B2B Workwear & Janitorial Limited (HOF-0050) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('B2B Workwear & Janitorial Limited'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('B2B Workwear & Janitorial Limited'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('B2B Workwear & Janitorial Limited')) || '%'
       OR LOWER('B2B Workwear & Janitorial Limited') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0050'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0050'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0050',
        dc_customer_id = 'HOF-0050',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- ── Raw Dog Store Ltd (HOF-0053) [HOF-Fallback] ──
  SELECT id INTO v_cust_id FROM customers 
  WHERE LOWER(TRIM(business_name)) = LOWER(TRIM('Raw Dog Store Ltd'))
  LIMIT 1;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE REGEXP_REPLACE(LOWER(business_name), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g') = 
          REGEXP_REPLACE(LOWER('Raw Dog Store Ltd'), '(limited|ltd|plc|llc|uk|t/a|[^a-z0-9])', '', 'g')
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;

  IF v_cust_id IS NULL THEN
    SELECT id INTO v_cust_id FROM customers 
    WHERE LOWER(business_name) LIKE '%' || LOWER(TRIM('Raw Dog Store Ltd')) || '%'
       OR LOWER('Raw Dog Store Ltd') LIKE '%' || LOWER(TRIM(business_name)) || '%'
    ORDER BY LENGTH(business_name) ASC LIMIT 1;
  END IF;
  ...
  IF v_cust_id IS NOT NULL THEN
    ...
    UPDATE customers 
    SET account_number = 'MOS-' || SUBSTRING(id::TEXT, 1, 8)
    WHERE account_number = 'HOF-0053'
      AND id != v_cust_id;

    UPDATE customers 
    SET dc_customer_id = NULL 
    WHERE dc_customer_id = 'HOF-0053'
      AND id != v_cust_id;

    UPDATE customers 
    SET account_number = 'HOF-0053',
        dc_customer_id = 'HOF-0053',
        updated_at = NOW()
    WHERE id = v_cust_id;
  END IF;

  -- Explicitly ensure Hairbitz Ltd is set to HOF-0029
  UPDATE customers
  SET account_number = 'HOF-0029',
      dc_customer_id = 'HOF-0029',
      updated_at = NOW()
