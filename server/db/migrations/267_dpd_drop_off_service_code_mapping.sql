...
--   3. Replace FILL_IN_PRODUCT_CODE in the INSERT below with that value.
--
-- Current mappings for reference:
--   courier_code='2', product_code=NULL  -> DPD-12  (Next Day, catch-all)
--   courier_code='3', product_code=NULL  -> DPD-12  (Next Day)
--
-- After adding the composite mapping the resolution order becomes:
--   courier_code='2', product_code=<DROP_CODE>  -> DPD12-DROP  (new)
--   courier_code='2', product_code=NULL         -> DPD-12      (fallback)
--
-- UNCOMMENT AND RUN ONCE THE PRODUCT CODE IS CONFIRMED:
--
...
--
-- NOTE: also consider whether DPD-12DROPQR and DPD-DROP5KND need their own
-- composite mappings, or whether a customer-specific override in the
-- Reconciliation -> Service Code Mappings UI is the right path.
