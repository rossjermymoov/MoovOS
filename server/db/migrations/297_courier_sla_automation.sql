-- 297_courier_sla_automation.sql
-- Courier SLA + autonomous-automation state for the Split-Timeline Courier
-- Orchestration Protocol. Adds two tracking columns to queries and surfaces
-- them (plus a computed breach flag) on the inbox view.

ALTER TABLE queries
  ADD COLUMN IF NOT EXISTS courier_sla_expires_at   TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS internal_automation_state TEXT;

-- Recreate the inbox view with the courier-SLA fields appended.
...
SELECT
  ...
  (SELECT COUNT(*) FROM query_emails qe
   WHERE qe.query_id = q.id AND qe.is_ai_draft = true AND qe.sent_at IS NULL AND qe.ai_draft_approved_by IS NULL
  ) AS pending_drafts,

  (SELECT qe.body_text FROM query_emails qe
   WHERE qe.query_id = q.id ORDER BY COALESCE(qe.received_at, qe.created_at) DESC LIMIT 1
  ...
  (SELECT COUNT(*) FROM query_emails qe
   WHERE qe.query_id = q.id AND qe.direction IN ('inbound_customer', 'inbound_courier')
     AND qe.read_at IS NULL AND qe.is_ai_draft = false
  ) AS unread_emails,
  ...
  COALESCE(sa.due_at, CASE WHEN s.sla_hours IS NOT NULL THEN q.created_at + (s.sla_hours || ' hours')::INTERVAL ELSE NULL END) AS sla_due_at,
  ...
  -- ── Courier SLA + automation state ──────────────────────────────────────
  q.courier_sla_expires_at,
  q.internal_automation_state,
  CASE
    WHEN q.courier_sla_expires_at IS NOT NULL
      AND q.internal_automation_state = 'awaiting_courier_response'
      AND NOW() > q.courier_sla_expires_at
      AND q.status NOT IN ('resolved', 'resolved_claim_approved', 'resolved_claim_rejected')
    THEN true ELSE false
  END AS courier_sla_breached
